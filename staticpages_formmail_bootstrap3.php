// +---------------------------------------------------------------------------+
// | FormMail Static Page for Geeklog 2.1 higher for Bootstrap3
// +---------------------------------------------------------------------------+
...
# 必須入力の文字列
$required_string = '<span class="text-warning">*</span>';
...
array('header'=>'法人様名',
  ...
  'data'=>array(
array( 'type'=>'text', 'name'=>'q_organization', 'id'=>'kaisha', 'maxlength'=>'60', 'class'=>'form-control ime_on', 'placeholder'=>'全角で入力してください。' ),
  ),
),
// } 1行
// 1行 {
...
array('header'=>'希望日',
  'data'=>array(
array( 'type'=>'text', 'name'=>'q_date1', 'size'=>'20', 'style'=>'width: 20em;', 'class'=>'form-control', 'data-uk-datepicker'=>"{format:'YYYY.MM.DD'}" ),
  ),
),
// } 1行
// 1行 {
...
array('header'=>'お問い合わせ内容',
  ...
  'data'=>array(
array( 'type'=>'textarea', 'name'=>'q_other', 'class'=>'form-control ime_on', 'style'=>'width: 95%; height: 100px;', 'onKeyup'=>"var n=500-this.value.length;var s=document.getElementById('tasp1');s.innerHTML='('+n+')';", 'placeholder'=>'お問い合わせ内容を入力してください。' ),
...
function _fmMkSeni ($items, $action) {
  ...
  foreach ($items as $key => $value) {
    if ($action == $key) {
      $buf .= '  <li><h4><span class="label label-default label-info">'.$value.'</span></h4></li>'.LB;
    } else {
      $buf .= '  <li><h4><span class="label label-default">'.$value.'</span></h4></li>'.LB;
    }
  }
  $buf .= '</ul>'.LB;
  return $buf;
